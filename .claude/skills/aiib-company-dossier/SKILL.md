...
worker's budget: **prefer WebSearch + `fetch_financials.py`; WebFetch a page only when a snippet is
insufficient.** **Cap the fan-out:** at most **~6 workers concurrently** — more companies than that,
dispatch in waves.
