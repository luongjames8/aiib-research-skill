...
description: >-
  ...
  B (aiib-company-dossier). Sits between Mode A and Mode B.
---
...
feeds the **aiib-company-dossier** mode (Mode B). Full method detail: `aiib-research/references/sourcing-methods.md`.
...
**Dedup the anchors BEFORE Step 2** (this is what stops successive rounds re-chasing the same names).
...
(`aiib-research/references/sourcing-methods.md`): graph expansion (competitors, suppliers, customers, JV partners,
...
2. **Round.** Spawn a **small batch (~3–4)** of `source-expander` workers, each given: one anchor/method
   ...
   in this list."*
3. **Dedup** the round's returns into `found` (`python aiib-research/scripts/dedup_candidates.py`).
...
expensive + often 403s) · **round cap ~4–5** even if not fully dry. On chat apps without subagent support,
run the same rounds sequentially yourself, within the budget.
...
Without filesystem/subagents, do the dedup by hand with the Step-1 normalization rule. Then apply a **light** mandate-fit gate (`aiib-research/references/aiib-mandate.md`
