...
description: >-
  ...
  Mode S (aiib-company-sourcing) instead. Stage one of the funnel; feeds Mode S then Mode B.
---

# AIIB Sector Scan (Mode A)

Top-down funnel, stage one: **Country · Sector → sub-sector economics + anchors.** Methodology proven on
a live deal-sourcing system. Output (economics + anchors) feeds the **aiib-company-sourcing** steering doc
(Mode S) → which feeds the **aiib-company-dossier** steering doc (Mode B).
...
- **Required:** a country and a sector. The sector should map to one of AIIB's 6 (Energy, Transport,
  Sustainable Cities, Digital Infrastructure, Water, Health) — see `aiib-research/references/aiib-mandate.md`. Accept
...
template (`aiib-research/references/ai-template.md`: market economics · pricing/tariff · margins · revenue quality ·
...
**Structured comps are automatic** (`aiib-research/references/data-sources.md`): the `subsector-researcher` workers
run `python aiib-research/scripts/fetch_financials.py <ticker>` themselves for field E — it **auto-installs
...
**No subagent tool (chat app without agent capabilities):** work through every sub-sector sequentially
yourself, same A–I template + per-sub-sector search budget.
...
`aiib-research/references/aiib-mandate.md` (cite it). Then surface the **anchors** that the sourcing stage expands
...
full private-company hunt here — that's the **aiib-company-sourcing** mode (Mode S), which graph-expands
...
1. **Header** — `<Country> · <Sector>` + coverage note (N sub-sectors covered) + a provenance banner
   (especially the ⚠️ no-live-sources banner if web access is unavailable — see `aiib-research/references/provenance.md`).
